...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        extra_kwargs = {
            "password": {"write_only": True}, #보안을 위해 사용자에게 비밀번호를 반환하지 않음
            "email": {"required": True},
            "username": {"required": False, "allow_blank": True},  # ERD를 기준으로 닉네임을 유저네임 대신 쓰기때문에 유저네임은 공백가능
        }
    ...
    def create(self, validated_data):
        password = validated_data.pop("password", None) #비밀번호가 없는 경우 none 리턴
